...
def main():
    # create form
    cform = CreateRecord(prefix='cform')
    dform = DeleteRecord(prefix='dform')
    uform = UpdateRecord(prefix='uform')
    # reset = Reset(prefix='reset')

    # response
    if cform.validate_on_submit() and cform.create.data:
        return createRecord(cform)
    if dform.validate_on_submit() and dform.delete.data:
        return deleteRecord(dform)
    if uform.validate_on_submit() and uform.update.data:
        return updateRecord(uform)
